...
Flutter 是真正意义上的原生跨平台开发框架，它完美的做到了一套代码，即可在 Android、iOS 两大移动平台运行。
...
- 由 **c/c++** 编写的 Flutter 引擎

- 由 **Dart** 编写的框架 API

Flutter 之所以能够在原生层面上做到跨平台，是因为它通过在底层使用通用的图形绘制库（[OpenGL](https://en.wikipedia.org/wiki/OpenGL)），封装了一套全新的、独立的 UI 框架，即 Flutter。
